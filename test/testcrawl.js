...
describe("Test Crawl", function () {
    ...
    it("should send multiple cookies properly", function (done) {
        ...
        crawler.on("fetchstart", function (queueItem, requestOptions) {
            requestOptions.headers.cookie.should.be.a("string");
            requestOptions.headers.cookie.should.match(
                /^(name\d=value\d; ){2}(name\d=value\d)$/
            );
    ...
    it("should be able to disregard rules in robots.txt", function (done) {
        ...
        crawler.on("complete", function () {
                ...
                new Error(
                    "Didn't visit forbidden URL (even though it should have)"
                )
            );
        });
    });
    ...
    it("should obey robots.txt on different hosts", function (done) {
        ...
        crawler.on("fetchdisallowed", function (parsedURL) {
            uri({
                ...
                .href()
                .should.equal("http://127.0.0.1:3001/disallowed");
    ...
    it("it should remove script tags if parseScriptTags is disabled", function (done) {
        ...
        crawler.on("complete", function () {
            crawler.queue.exists(
                "http://127.0.0.1:3000/not/existent/file.js",
    ...
    it("it should emit an error when resource is too big", function (done) {
        ...
        crawler.on("fetchdataerror", function (queueItem) {
            visitedUrl =
                visitedUrl || queueItem.url === "http://127.0.0.1:3000/big";
        });
    ...
    // NOTE: this test does not work. It only adds /sitemap.xml URL to the queue,
    // but it should add https://localhost:3000/
    it.skip("should allow initial redirect to different domain if configured", function (done) {
        ...
        crawler.on("complete", function () {
            if (queueaddCount <= 1) {
                    ...
                    new Error(
                        "Didn't visit redirected URL (even though it should have)"
                    )
                );
            }
        });
