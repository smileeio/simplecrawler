...
var Crawler = function (initialURL) {
    ...
    if (arguments.length > 1) {
        throw new Error(
            "Since 1.0.0, simplecrawler takes a single URL when initialized. Protocol, hostname, port and path are inferred from that argument."
        );
    }

    if (typeof initialURL !== "string") {
        throw new Error(
            "The crawler needs a URL string to know where to start crawling"
        );
    }
...
Crawler.prototype.start = function () {
    ...
    crawler.queue.add(queueItem, false, function (error) {
        ...
        process.nextTick(function () {
            crawler.crawlIntervalID = setInterval(
...
Crawler.prototype.getRequestOptions = function (queueItem) {
    ...
    var agent =
        queueItem.protocol === "https" ? crawler.httpsAgent : crawler.httpAgent;
    ...
    var isStandardHTTPPort =
            queueItem.protocol === "http" && queueItem.port !== 80,
        isStandardHTTPSPort =
            queueItem.protocol === "https" && queueItem.port !== 443,
    ...
    var requestOptions = {
        ...
        headers: {
            ...
            Host:
                queueItem.host +
                (queueItem.port && isStandardPort ? ":" + queueItem.port : ""),
        },
    };
    ...
    if (crawler.acceptCookies && crawler.cookies.getAsHeader()) {
        requestOptions.headers.cookie = crawler.cookies
            .getAsHeader(queueItem.host, queueItem.path)
            .join("; ");
    }
...
Crawler.prototype.getRobotsTxt = function (url, callback) {
    ...
    var requestOptions = crawler.getRequestOptions(
        crawler.processURL(robotsTxtUrl.href())
    );
    ...
    var clientRequest = client.request(requestOptions, function (response) {
        if (response.statusCode >= 200 && response.statusCode < 300) {
            ...
            response.on("data", function (chunk) {
                ...
                if (
                    ...
                    if (
                        ...
                        response.destroy();
                        callback(
                            new Error("robots.txt exceeded maxResourceSize")
                        );
                    }
                } else {
                    chunk.copy(
            ...
            var decodeAndReturnResponse = function (error, responseBuffer) {
                if (error) {
                    return callback(
                        new Error("Couldn't unzip robots.txt response body")
                    );
                }
                ...
                var contentType = response.headers["content-type"],
                    responseBody = crawler.decodeBuffer(
                        responseBuffer,
                        contentType
                    );
            ...
            try {
                ...
                errorMsg = util.format(
                    "Faulty redirect URL when fetching robots.txt for %s",
                    robotsTxtHost
                );
            ...
            if (crawler.domainValid(redirectTarget.hostname())) {
                ...
                errorMsg = util.format(
                    "%s redirected to a disallowed domain (%s)",
                    robotsTxtUrl.href(),
            ...
            errorMsg = util.format(
                "Server responded with status %d when fetching robots.txt",
                response.statusCode
            );
            callback(new Error(errorMsg));
        }
...
Crawler.prototype.discoverResources = function (resourceText) {
    ...
    if (
        ...
        /<meta(?:\s[^>]*)?\sname\s*=\s*["']?robots["']?[^>]*>/i.test(
            resourceText
        )
    ) {
        var robotsValue =
            /<meta(?:\s[^>]*)?\scontent\s*=\s*["']?([\w\s,]+)["']?[^>]*>/i.exec(
                resourceText.toLowerCase()
            );
...
Crawler.prototype.domainValid = function (host) {
    ...
    function isSubdomainOf(subdomain, host) {
        ...
        if (crawler.ignoreWWWDomain) {
            subdomain = subdomain.replace(/^www./gi, "");
            host = host.replace(/^www./gi, "");
        }
...
Crawler.prototype.queueLinkedItems = function (resourceData, queueItem) {
    ...
    var resources = crawler.discoverResources(
...
Crawler.prototype.queueURL = function (url, referrer, force) {
    var crawler = this,
        queueItem =
            typeof url === "object" ? url : crawler.processURL(url, referrer);
    ...
    async.every(
        ...
        function (fetchCondition, callback) {
            ...
            clientRequest.on("abort", function () {
                if (crawler._openRequests.indexOf(clientRequest) > -1) {
                    crawler._openRequests.splice(
                        crawler._openRequests.indexOf(clientRequest),
                        1
                    );
                }
            });
...
Crawler.prototype.decodeBuffer = function (buffer, contentTypeHeader) {
    ...
    var embeddedEncoding =
            /<meta[^>]*charset\s*=\s*["']?([\w-]*)/i.exec(
                buffer.toString(undefined, 0, 512)
            ) || [],
        encoding =
            contentTypeHeader.split("charset=")[1] ||
            embeddedEncoding[1] ||
            contentTypeHeader;
 ...
 * @return {Crawler}                         Returns the crawler instance to enable chained API calls
 */
Crawler.prototype.handleResponse = function (
    queueItem,
    response,
    timeCommenced
) {
            ...
            if (responseLength > crawler.maxResourceSize) {
                crawler.queue.update(
                    ...
                    function (error, queueItem) {
                        ...
                        // Remove this request from the open request map
                        crawler._openRequests.splice(
                            crawler._openRequests.indexOf(response.req),
                            1
                        );
                ...
                async.every(
                    crawler._downloadConditions,
                    function (downloadCondition, callback) {
                        if (downloadCondition === undefined) {
                            ...
                            try {
                                callback(
                                    null,
                                    downloadCondition(queueItem, response)
                                );
                            } catch (error) {
                                callback(error);
                            }
                ...
                crawler.queue.update(
                    ...
                    function (error, queueItem) {
                        ...
                        redirectQueueItem = crawler.processURL(
                            response.headers.location,
                            queueItem
                        );
                ...
                crawler.queue.update(
                    ...
                    function (error, queueItem) {
                        ...
                        if (
                            ...
                            crawler.queueLinkedItems(
                                decompressedBuffer || responseBody,
                                queueItem
                            );
                        }
                    }
            ...
            function processReceivedData() {
                ...
                crawler.queue.update(
                    queueItem.id,
                    {
                        stateData: {
                            downloadTime:
                                timeDataReceived - timeHeadersReceived,
...
Crawler.prototype.crawl = function () {
    ...
    if (
        crawler._openRequests.length >= crawler.maxConcurrency ||
        crawler.fetchingRobotsTxt ||
        crawler.fetchingQueueItem
    ) {
        return crawler;
    }
    ...
    crawler.queue.oldestUnfetchedItem(function (error, queueItem) {
        ...
        if (error) {
            ...
            if (
                crawler.respectRobotsTxt &&
                crawler._touchedHosts.indexOf(host) === -1
            ) {
                ...
                crawler.getRobotsTxt(
                    robotsTxtUrl,
                    function (error, robotsTxtUrl, robotsTxtBody) {
                        if (error) {
                            /**
                            ...
                            var robotsQueueItem = crawler.processURL(
                                robotsTxtUrl,
                                queueItem
                            );
